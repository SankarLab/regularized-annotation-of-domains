...
def sample_weights(val_df):
    ...
    num_classes = len(np.unique(val_df['target'].values))
    for i, grp in df.groupby('target'):
        indices[grp.index.to_list()] = len(df)/(num_classes * len(df[(df['target'] == i)]))
    return indices
...
for dataset in datasets:
    ...
    for noise_level in noise:
       ...
       for exp in range(10):
            ...
            for c in C_VALUES:
                ...
                results.loc[len(results)] = {'C':c,'alpha':None,'n':None,'val_wga':dfr_val,'test_wga':dfr_test,'type':'CUW'}
                
                
            dfr_avg_param = results[results['type'] == 'CUW'].groupby(['C'])['test_wga'].mean().idxmax()
            ...
            print("LLR - class-upweighted (" + dataset + ")(" + str(noise_level) + "): ", dfr_wga.mean(), dfr_wga.std())
    ...
    path = 'results/hparams_CUW_' + dataset + '.csv'
