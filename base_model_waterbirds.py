...
epochs = 100
THRESHOLD = 0.5
PATH = 'waterbirds_low_lr_resnet50.pt'
log_every = 30

data = pd.read_csv('/home/rayyaga2/Thesis work/OOD_analysis/fine_tuned_OOD/wb_data/waterbird_complete95_forest2water2/wb_metadata.csv')


source_trainset = imgDataset(data, split=0, augment_data_flag = True, root_dir = '/home/rayyaga2/Thesis work/OOD_analysis/fine_tuned_OOD/wb_data/waterbird_complete95_forest2water2') #splits are as follows: 
#source_testset = CelebADataset(data, split=1)  # 0 -> source training, 1 -> source testing, 2 -> target training, 3 -> target testing

source_trainloader = DataLoader(source_trainset, batch_size=32, shuffle=True)
...
torch.save(model, 'waterbirds_entire_model.pt')