...
from data import group_sln, group_balance
...
c_values = [0.379269, 0.143845, 0.004833, 0.004833, 0.000695]
...
for dataset in datasets:
    ...
    for i, noise_level in enumerate(noise):
        ...
        print("Group Downsampled LLR (" + dataset + ")(" + str(noise_level) + "): ", dfr_wga.mean(), dfr_wga.std())
        final_results.loc[len(final_results)] = {'dataset': dataset, 'noise': noise_level, 'wga_mean': dfr_wga.mean(), 'wga_std': dfr_wga.std(), 'C': c } 
        
    path = 'results/final_group_downsampled_' + dataset + '.csv'
