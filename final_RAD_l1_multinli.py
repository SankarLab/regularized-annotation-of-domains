...
final_results = pd.DataFrame(columns=['dataset', 'noise', 'wga_mean', 'wga_std', 'c_sel', 'lr_sel', 'epochs_sel', 'c', 'lamda'])
...
for dataset in datasets:
    ...
    for i, seed in enumerate(seeds):
        ...
        _,rad_wga[i] = run_misclassify_upweight(full_train_data, final_test_data, c, lamda, epochs_sel, lr_sel, w_decay_sel, c_sel)
        print(rad_wga[i])
    
    print("RAD (" + dataset + ")(" + str(noise_level) + "): ", rad_wga.mean(), rad_wga.std())
    final_results.loc[len(final_results)] = {'dataset': dataset, 'noise': noise_level, 'wga_mean': rad_wga.mean(), 'wga_std': rad_wga.std(), 'c_sel': c_sel, 'lr_sel': lr_sel, 'epochs_sel': epochs_sel, 'c': c, 'lamda': lamda}
    
    path = 'results/final_RAD_L1_' + dataset + '.csv'
