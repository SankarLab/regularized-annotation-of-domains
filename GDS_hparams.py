...
from data import group_balance, group_sln
    ...
    for noise_level in noise:
       ...
       for exp in range(10):
            ...
            for c in C_VALUES:
                ...
                results.loc[len(results)] = {'C':c,'alpha':None,'n':None,'val_wga':dfr_val,'test_wga':dfr_test,'type':'DFR'}
                
                
            dfr_avg_param = results[results['type'] == 'DFR'].groupby(['C'])['test_wga'].mean().idxmax()
            ...
            print("DFR (" + dataset + ")(" + str(noise_level) + "): ", dfr_wga.mean(), dfr_wga.std())
            ...
            results = results[0:0] 
    path = 'results/hparams_GDS_' + dataset + '.csv'
