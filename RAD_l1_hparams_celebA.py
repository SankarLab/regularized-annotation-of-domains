...
from data import group_sln, reg_misclassification_detection, wga


def run_misclassify_upweight(val_data, test_data, c, lam, epochs_sel, lr_sel, w_decay_sel, c_sel):
    data = val_data.copy()
    detector = reg_misclassification_detection(val_data, test_data)
    data_ds = detector.run_detection_model(epochs=epochs_sel, lr=lr_sel, weight_decay=w_decay_sel, c_sel=c_sel, opt='AdamW', lr_scheduler = 'cosine')
    ...
    model = LogisticRegression(penalty = 'l1', solver='liblinear',C=c).fit(full_data.drop(['target','group'],axis=1), full_data['target'], weight)
...
datasets = ['celebA']
noise = [0]
final_results = pd.DataFrame(columns=['dataset', 'noise', 'wga_mean', 'wga_std', 'C', 'lambda', 'iters', 'lr', 'c_sel', 'exp'])
C_VALUES = np.logspace(-4,0,num=20, base=10)
C_SEL_VALUES = np.logspace(-7,-3,num=20, base=10)
LR_VALUES = [1e-5]
LAMBDA_VALUES= np.linspace(20, 40, 5)
ITER_VALUES = [6]
...
for dataset in datasets:
    ...
    for noise_level in noise:
       ...
       for exp in range(10):
            ...
            results = pd.DataFrame(columns=['C', 'lambda', 'iters', 'lr', 'c_sel', 'val_wga','test_wga','type'])
                    ...
                    for lam in LAMBDA_VALUES:
                        for i in ITER_VALUES:
                            for c_sel in C_SEL_VALUES:
                                print(c,lr_sel,lam,i,c_sel)
                                rad_val, rad_test = run_misclassify_upweight(train_data,test_data, c, lam, i, lr_sel, w_decay_sel, c_sel)
                                results.loc[len(results)] = {'C':c,'lambda':lam,'iters':i,'lr':lr_sel, 'c_sel': c_sel, 'val_wga':rad_val,'test_wga':rad_test,'type':'RAD'}

            rad_avg_param = results[results['type']=='RAD'].groupby(['C','iters','lr','lambda', 'c_sel'])['test_wga'].mean().idxmax()
           
            print(rad_avg_param)

            rad_wga = np.zeros(10)
            for i in range(10):
                _,rad_wga[i] = run_misclassify_upweight(full_train_data, final_test_data, rad_avg_param[0], rad_avg_param[3], rad_avg_param[1], rad_avg_param[2], w_decay_sel, rad_avg_param[4])
            
            
            print("RAD (" + dataset + ")(" + str(noise_level) + "): ", rad_wga.mean(), rad_wga.std())
            final_results.loc[len(final_results)] = {'dataset': dataset, 'noise': noise_level, 'wga_mean': rad_wga.mean(), 'wga_std': rad_wga.std(), 'C': rad_avg_param[0], 'lambda':rad_avg_param[3], 'iters': rad_avg_param[1], 'lr':rad_avg_param[2], 'c_sel': rad_avg_param[4], 'exp': exp } 
            results = results[0:0] 

final_results.to_csv('results/hparams_RAD_l1_celebA.csv', mode='a', header=True)